...
export default function Checkout() {
  ...
  const plans = lp?.content?.pricing?.plans || []

  // Aparência herdada do template (bg + texto)
  const bgKey = useMemo(() => `plp:lp:${lp?.id ?? "unknown"}:saas:bg`, [lp?.id])
  const fgKey = useMemo(() => `plp:lp:${lp?.id ?? "unknown"}:saas:fg`, [lp?.id])
  const [bg, setBg] = useState("#0f172a")
  const [fg, setFg] = useState("#ffffff")
  useEffect(() => {
    try {
      const sbg = localStorage.getItem(bgKey)
      const sfg = localStorage.getItem(fgKey)
      if (sbg) setBg(sbg)
      if (sfg) setFg(sfg)
    } catch { /* ignore */ }
  }, [bgKey, fgKey])
      ...
      <div className="mx-auto max-w-5xl px-6 py-10 lg:py-16">
        ...
        <div className="grid gap-6 md:grid-cols-3">
              ...
              <Section title="Plano">
                <div className="grid gap-1.5 sm:grid-cols-3">
                  ...
                  <div className="grid gap-1.5">
                    ...
                    <div className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 flex items-center dark:border-slate-700 dark:bg-slate-900">
            ...
            {mode !== "form" && (
              <Section title={mode === "register" ? "Criar conta" : "Entrar"}>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="grid gap-1.5">
                    <label className="text-sm">E-mail/Usuário</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="Seu e-mail" />
                  </div>
                  <div className="grid gap-1.5">
                    <label className="text-sm">Senha</label>
                    <input type="password" className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={password} onChange={(e)=>setPassword(e.target.value)} placeholder="••••••••" />
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {mode === "register" ? (
                    <button type="button" onClick={()=>{ try { register(email, password); setAuthErr("") } catch(e){ setAuthErr(e.message) } }} className="rounded-lg bg-sky-600 px-4 py-2 text-white">Criar conta</button>
                  ) : (
                    <button type="button" onClick={()=>{ try { login(email, password); setAuthErr("") } catch(e){ setAuthErr(e.message) } }} className="rounded-lg bg-sky-600 px-4 py-2 text-white">Entrar</button>
                  )}
              ...
              <Section title="Seus dados">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="grid gap-1.5">
                    ...
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.nome} onChange={(e)=>onChange("nome", e.target.value)} />
                  </div>
                  <div className="grid gap-1.5">
                    <label className="text-sm">CPF</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.cpf} onChange={(e)=>onChange("cpf", e.target.value)} placeholder="000.000.000-00" />
                  </div>
                  <div className="grid gap-1.5">
                    <label className="text-sm">Telefone</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.telefone} onChange={(e)=>onChange("telefone", e.target.value)} placeholder="(00) 90000-0000" />
                  </div>
                </div>
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="grid gap-1.5 sm:col-span-2">
                    <label className="text-sm">Endereço</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.endereco} onChange={(e)=>onChange("endereco", e.target.value)} placeholder="Rua" />
                  </div>
                  <div className="grid gap-1.5">
                    <label className="text-sm">Número</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.numero} onChange={(e)=>onChange("numero", e.target.value)} />
                  </div>
                </div>
                <div className="grid gap-3 sm:grid-cols-4">
                  <div className="grid gap-1.5 sm:col-span-2">
                    <label className="text-sm">Complemento</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.complemento} onChange={(e)=>onChange("complemento", e.target.value)} />
                  </div>
                  <div className="grid gap-1.5">
                    <label className="text-sm">Bairro</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.bairro} onChange={(e)=>onChange("bairro", e.target.value)} />
                  </div>
                  <div className="grid gap-1.5">
                    <label className="text-sm">CEP</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.cep} onChange={(e)=>onChange("cep", e.target.value)} placeholder="00000-000" />
                  </div>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="grid gap-1.5">
                    <label className="text-sm">Cidade</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.cidade} onChange={(e)=>onChange("cidade", e.target.value)} />
                  </div>
                  <div className="grid gap-1.5">
                    <label className="text-sm">UF</label>
                    <input className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" value={form.uf} onChange={(e)=>onChange("uf", e.target.value)} placeholder="SP" />
                  </div>
                </div>
                ...
                <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    <input className="h-11 flex-1 rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" placeholder="Ex.: CUPOM10" value={coupon} onChange={(e)=>setCoupon(e.target.value)} />
                    <button type="button" className="rounded-lg border border-slate-300 bg-white px-3 dark:border-slate-700 dark:bg-slate-900" onClick={applyCoupon}>Aplicar</button>
                  </div>
                  {applied && (
          ...
          <aside className="md:col-span-1">
            <div className="sticky top-6 rounded-2xl border border-slate-200 bg-white/70 p-6 shadow-md backdrop-blur dark:border-slate-800 dark:bg-slate-900/60">
              <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100">Resumo</h3>
              ...
              <button
                disabled={mode !== "form"}
                onClick={goPayment}
                className="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-sky-600 px-4 py-2 text-white shadow-sm transition hover:bg-sky-700 disabled:opacity-50 dark:bg-sky-500 dark:hover:bg-sky-600"
              >
