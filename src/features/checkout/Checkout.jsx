...
export default function Checkout() {
  ...
  const bgKey = useMemo(
    () => (isSaaS ? `plp:lp:${lp?.id ?? 'unknown'}:saas:bg` : null),
    [isSaaS, lp?.id],
  );
  ...
  const [basePrice, setBasePrice] = useState(
    priceFromQ || fallbackPlanPrice.price,
  );
  useEffect(() => {
    ...
    cepDebounceRef.current = setTimeout(async () => {
      try {
        ...
        setCepInfo({
          status: 'ok',
          message:
            `${data.logradouro || ''} ${data.bairro ? '- ' + data.bairro : ''} ${data.localidade || ''}/${data.uf || ''}`.trim(),
        });
      } catch {
  ...
  const applyCoupon = () => {
    ...
    if (!c) {
      setApplied({ code, amount: 0, invalid: true });
      return;
    }
    let discount = 0;
  ...
  useEffect(() => {
    ...
    setApplied({
      code: applied.code,
      amount: Math.min(discount, basePrice),
      invalid: false,
    });
  ...
  const goPayment = () => {
    ...
    const order = createOrder({
      lp,
      amount: finalPrice,
      descricao_oferta: `${lp?.titulo || 'Oferta'}${planName ? ` · ${planName}` : ''}`,
    });
      ...
      <div className="mx-auto max-w-5xl px-6 py-10 lg:py-16">
        <header className="mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-300">
            {lp.titulo} · {planName ? `Plano: ${planName}` : ''}
          </p>
        </header>
          ...
          <div className="md:col-span-2 grid gap-6">
            ...
            {mode !== 'form' && (
              <Section title={mode === 'register' ? 'Criar conta' : 'Entrar'}>
                {authErr && (
                  <div className="rounded border border-red-200 bg-red-50 p-2 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-200">
                    {authErr}
                  </div>
                )}
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="grid gap-1.5">
                    ...
                    <input
                      className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-slate-900 placeholder-slate-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-400"
                      value={email}
              ...
              <Section title="Seus dados">
                ...
                <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      className="h-11 flex-1 rounded-lg border border-slate-300 bg-white px-3 text-slate-900 placeholder-slate-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-400"
                    ...
                    <button
                      type="button"
                      className="rounded-lg border bg-white px-3 dark:bg-slate-900"
                  ...
                  {applied && (
                    <div
                      className={`text-sm ${applied.invalid ? 'text-red-600' : 'text-emerald-600'}`}
                    >
                      {applied.invalid
                        ? 'Cupom inválido'
                        : `Cupom aplicado: -${currencyBRL(applied.amount)}`}
                    </div>
                  )}
                </div>
              </Section>
            ...
            <div className="sticky top-6">
              <Section title="Resumo">
                <div className="grid gap-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span>Subtotal</span>
                    <span>{currencyBRL(basePrice)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Descontos</span>
                    <span>-{currencyBRL(applied?.amount || 0)}</span>
                  </div>
                  <div className="mt-2 flex justify-between font-medium">
                    <span>Total</span>
                    <span>{currencyBRL(finalPrice)}</span>
                  </div>
                </div>
                <button
                  disabled={mode !== 'form'}
                ...
                <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                  Os dados de pagamento serão solicitados na próxima etapa.
                </p>
              </Section>
            </div>
          </aside>
