...
function PromoBanner({ theme, data = {}, lp }) {
  ...
  const onClick = () =>
    logEvent('cta_click', { lp_id: lp?.id, cta_id: 'banner', target: btnHref });
    ...
    <Section id="banner">
      <div className="relative w-full">
        {hasImg ? (
          ...
          <div
            className="grid w-full place-items-center border-y border-dashed border-slate-300 text-slate-400 dark:border-slate-700"
            style={{ height: heightPx }}
          >
      ...
      <Container>
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {data.map((f, i) => (
            <div
              key={i}
              className="rounded-xl border border-slate-200 p-4 transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800"
            >
  ...
  const list = steps.length
    ...
    : [
        ...
        },
        {
          ...
          text: 'Selecione o que combina com seu negócio.',
          icon: '🎨',
        },
        {
          title: 'Publique e acompanhe',
          text: 'Meça conversões em tempo real.',
          icon: '📈',
        },
      ];
  return (
      ...
      <Container>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {list.map((s, i) => (
            <div
              key={i}
              className="rounded-xl border border-slate-200 p-5 transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800"
            >
    ...
    <Section id="pricing">
      <Container>
        ...
        <p className="mb-6 text-slate-600 dark:text-slate-300">
          {data.subtitle ?? 'Escolha o plano ideal.'}
        </p>
        <div className="grid gap-4 md:grid-cols-3">
          {plans.map((p) => (
            <div
              key={p.name}
              className={
                'rounded-xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md ' +
                (p.featured ? 'ring-1 ring-offset-1' : '')
              }
              style={
                p.featured
                  ? { borderColor: theme, boxShadow: `0 0 0 1px ${theme}` }
                  : {}
              }
            >
              <div className="font-medium text-slate-900 dark:text-slate-100">
          ...
          {list.map((src, i) =>
            src ? (
              <img
                ...
                src={src}
                alt="screenshot"
                className="h-40 w-auto rounded-lg border border-slate-200 shadow-sm dark:border-slate-800"
              />
            ) : (
              <div
                key={i}
                className="grid h-40 w-64 flex-shrink-0 place-items-center rounded-lg border border-dashed border-slate-300 text-slate-400 dark:border-slate-700"
              >
                Imagem {i + 1}
              </div>
            ),
