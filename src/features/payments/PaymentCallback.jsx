...
export default function PaymentCallback() {
  ...
  useEffect(() => {
    ...
    if (status === 'success' && updated) {
      logEvent('conversion', {
        lp_id: updated.lp_id,
        goal_id: 'payment_success',
        valor: updated.valor_total,
      });
    }
  }, [orderId, status]);
    ...
    <div className="min-h-screen p-6 grid place-items-center">
      <div className="w-full max-w-lg border rounded-xl p-6 grid gap-4">
        <h1 className="text-2xl">
          Pagamento {ok ? 'aprovado' : 'não concluído'}
        </h1>
        <div className="grid gap-1">
          <div>
            <b>Pedido:</b> {current.id_pedido}
          </div>
          <div>
            <b>Status:</b> {current.status}
          </div>
          <div>
            <b>Valor:</b> R$ {Number(current.valor_total || 0).toFixed(2)}
          </div>
        </div>
        <div className="flex gap-2">
          <Link className="border rounded px-3 py-2" to="/metrics">
            Ir para Métricas
          </Link>
