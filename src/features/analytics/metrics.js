...
export function computeMetrics(events) {
  const byVisitor = new Map(), bySession = new Map(), bySource = new Map()
  let pageViews = 0, clicks = 0, submits = 0, conversions = 0
  ...
  for (const sessEvents of bySession.values()) {
    ...
    const interacted = sessEvents.some(e =>
      e.type === "cta_click" || e.type === "form_start" || e.type === "form_submit" || e.type === "conversion"
    )
  ...
  sources.sort((a,b)=> (b.conversions - a.conversions) || (b.page_views - a.page_views))

  return {
    visitors, sessions, pageViews, clicks, submits, conversions,
    ctr, convRateSessions, convRateVisitors, bounceRate, sources,
  }
}
