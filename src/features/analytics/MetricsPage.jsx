...
export default function MetricsPage() {
  ...
  const filtered = useMemo(
    () =>
      events.filter((e) => (lpId ? String(e.lp_id) === String(lpId) : false)),
    [events, lpId],
  );
  ...
  const exportSourcesCsv = () => {
    if (!lpId) return;
    exportCSV(`sources_${start}_${end}_${lpId}.csv`, m.sources, [
      'source',
      'visitors',
      'sessions',
      'page_views',
  ...
  const exportPdf = () => {
    if (lpId)
      exportMetricsPDF(
        `relatorio_${start}_${end}_${lpId}.pdf`,
        m,
        m.sources,
        period,
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900">
      <div className="mx-auto max-w-6xl px-6 py-10 lg:py-16">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-slate-600 dark:text-slate-300">
                Período: {period || '—'} ·{' '}
                {lpId
                  ? `LP selecionada: ${lps.find((x) => String(x.id) === String(lpId))?.titulo ?? lpId}`
                  : 'Selecione uma landing page'}
                .
              </p>
            </div>
            <button
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="grid gap-1.5">
              ...
              <DateInput
                ...
                max={end ? fmt(addDays(parse(end), -1)) : undefined}
              />
            </div>
            <div className="grid gap-1.5">
              <label
              ...
              <DateInput
                ...
                min={start ? fmt(addDays(parse(start), 1)) : undefined}
              />
            </div>
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card
            title="Conv/Sessão"
            value={lpId ? pct(m.convRateSessions) : '—'}
          />
          <Card
            title="Conv/Visitante"
            value={lpId ? pct(m.convRateVisitors) : '—'}
          />
          <Card
            title="Taxa de Rejeição"
            value={lpId ? pct(m.bounceRate) : '—'}
          />
        </section>
...
function Card({ title, value }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white/70 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
      ...
      <div className="mt-2 text-2xl font-semibold text-slate-600 dark:text-white">
        {value}
      </div>
    </div>
  );
...
function Th({ children }) {
  return (
    <th className="p-3 text-left text-sm font-medium text-slate-700 dark:text-slate-200">
      {children}
    </th>
  );
}
...
function Td({ children, colSpan }) {
  return (
    <td colSpan={colSpan} className="p-3 text-slate-900 dark:text-slate-100">
      {children}
    </td>
  );
}
...
function DateInput({ id, value, onChange, min, max }) {
  ...
  const setMonthSafe = (y, m) => {
    ...
    if (maxDate && d > new Date(maxDate.getFullYear(), maxDate.getMonth(), 1))
      d = new Date(maxDate.getFullYear(), maxDate.getMonth(), 1);
    setMonth(d);
  };
  const onPrevMonth = () =>
    setMonthSafe(month.getFullYear(), month.getMonth() - 1);
  const onNextMonth = () =>
    setMonthSafe(month.getFullYear(), month.getMonth() + 1);
  const onMonthSelect = (e) =>
    setMonthSafe(month.getFullYear(), parseInt(e.target.value, 10));
  const onYearSelect = (e) =>
    setMonthSafe(parseInt(e.target.value, 10), month.getMonth());

  const currentYear = month.getFullYear();
  const baseYear = new Date().getFullYear();
  const minYear = Math.max(
    MIN_YEAR,
    minDate ? minDate.getFullYear() : MIN_YEAR,
  );
      ...
      {open &&
        createPortal(
          <div
            ...
            className="fixed z-[999999] w-[320px] rounded-xl border border-slate-200 bg-white p-3 shadow-lg dark:border-slate-800 dark:bg-slate-900"
          >
            <div className="mb-3 flex items-center justify-between gap-2">
              ...
              >
                {monthNames.map((name, idx) => (
                  <option className="capitalize" key={idx} value={idx}>
                    {name}
                  </option>
                ))}
              </select>
