...
function buildDocument(lp) {
  ...
  const metaDescription = escapeHtml(
    description || plainText(lp.content?.subhead || lp.content?.body || ''),
  );
...
function buildDefaultExport(lp, ctx = {}) {
  ...
  const heroImage = heroImageSrc
    ? `<div class="lp-hero-media"><img src="${escapeAttr(heroImageSrc)}" alt="Imagem em destaque" /></div>`
    : '';
...
function buildSaasExport(lp, ctx = {}) {
  ...
  const navbar = {
    ...
    links: Array.isArray(source.navbar?.links)
      ? source.navbar.links
      : Array.isArray(base.navbar?.links)
        ? base.navbar.links
        : [],
  ...
  const footerNote =
    source.footer?.note ??
    base.footer?.note ??
    'Copyright Perfect Landing Page';
...
<header class="lp-navbar">
  ...
  const heroImage = heroImagePath
    ? `<div class="lp-hero-media"><img src="${escapeAttr(heroImagePath)}" alt="Imagem do produto" /></div>`
    : '';
  sections.push(`
...
<section class="lp-section">
  ...
  if (screenshots.length > 0) {
    const shotPaths = screenshots
      .map((src, index) =>
        addAsset(src, `${lp.id || 'lp'}-screenshot-${index + 1}`),
      )
      .filter(Boolean);
...
function buildEventoExport(lp) {
  ...
  const footerNote =
    source.footer?.note ?? base.footer?.note ?? 'Reserve sua vaga.';
